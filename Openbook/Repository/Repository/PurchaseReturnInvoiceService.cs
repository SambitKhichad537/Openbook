...
{
	public class PurchaseReturnInvoiceService : IPurchaseReturn
    {
        private readonly ApplicationDbContext _context;
        private readonly DatabaseConnection _conn;
        private string tenantId;

        public PurchaseReturnInvoiceService(ApplicationDbContext context, DatabaseConnection conn, IServicioTenant servicioTenant)
        {
            _context = context;
            _conn = conn;
        ...
        public bool BillnoCheckExistence(string VoucherNo)
        {
            using (SqlConnection sqlcon = new SqlConnection(_conn.DbConn))
            {
                ...
                return sqlcon.Query<bool>("SELECT VoucherNo FROM PurchaseReturnMaster where VoucherNo=@VoucherNo AND tenantId=@tenantId", para, null, true, 0, CommandType.Text).SingleOrDefault();
            }
        }
        ...
        public async Task<bool> Delete(PurchaseReturnMaster master)
        {
            ...
            try
            {
                ...
                SqlCommand cmd = new SqlCommand("PurchaseReturnDelete", sqlcon);
        ...
        public async Task<int> Draft(PurchaseReturnMaster master)
        {
			SqlConnection sqlcon = new SqlConnection(_conn.DbConn);
			sqlcon.Open();
			...
			try
			{
				...
				sqlcon.Execute("DECLARE @ReturnValue int INSERT INTO PurchaseReturnMaster (SerialNo,VoucherNo,WarehouseId,OrderNo,Date,CustomerSupplierId,Narration,PurchaseMasterId,NetAmounts,TotalTax,DisPer,DiscountType,BillDiscount,GrandTotal,TotalAmount,PreviousDue,VoucherTypeId,UserId,FinancialYearId,BalanceDue,Status,PayAmount,AddedDate,TenantId,Reference)VALUES(@SerialNo,@VoucherNo,@WarehouseId,@OrderNo,@Date,@CustomerSupplierId,@Narration,@PurchaseMasterId,@NetAmounts,@TotalTax,@DisPer,@DiscountType,@BillDiscount,@GrandTotal,@TotalAmount,@PreviousDue,@VoucherTypeId,@UserId,@FinancialYearId,@BalanceDue,@Status,@PayAmount,@AddedDate,@TenantId,@Reference) SELECT @ReturnValue = SCOPE_IDENTITY() set @MemIDOUT =SCOPE_IDENTITY()", para, sql, 0, CommandType.Text);
				...
				if (Id > 0)
				{
					...
					foreach (var item in master.listOrder)
					{
						...
						paraOpening.Add("@PurchaseReturnMasterId", Id);
						...
						sqlcon.Execute("DECLARE @ReturnValue int INSERT INTO PurchaseReturnDetails (PurchaseReturnMasterId,PurchaseDetailsId,ProductId,Qty,Rate,UnitId,Discount,DiscountAmount,TaxId,BatchId,TaxAmount,GrossAmount,NetAmount,LedgerId,ProjectId,TaxRate,Amount,Description,TenantId)VALUES(@PurchaseReturnMasterId,@PurchaseDetailsId,@ProductId,@Qty,@Rate,@UnitId,@Discount,@DiscountAmount,@TaxId,@BatchId,@TaxAmount,@GrossAmount,@NetAmount,@LedgerId,@ProjectId,@TaxRate,@Amount,@Description,@TenantId) SELECT @ReturnValue = SCOPE_IDENTITY() set @DId =SCOPE_IDENTITY()", paraOpening, sql, 0, CommandType.Text);
		...
		public async Task<List<PurchaseReturnMasterView>> PurchaseReturnInvoiceFilter( DateTime fromDate, DateTime toDate, int supplierid, string strVoucherNo, string strStatus , string strFiltetype)
        {
			using (SqlConnection sqlcon = new SqlConnection(_conn.DbConn))
			{
				var para = new DynamicParameters();
				...
				para.Add("@TenantId", tenantId);
				var ListofPlan = sqlcon.Query<PurchaseReturnMasterView>("PurchaseReturnFilter", para, null, true, 0, commandType: CommandType.StoredProcedure).ToList();
				return ListofPlan;
			}
		}
		public async Task<List<PurchaseReturnMasterView>> PurchaseReturnInvoice(string strStatus , string strFiltetype)
		{
			using (SqlConnection sqlcon = new SqlConnection(_conn.DbConn))
			{
				var para = new DynamicParameters();
				...
				para.Add("@FilterType", strFiltetype);
				para.Add("@TenantId", tenantId);
				var ListofPlan = sqlcon.Query<PurchaseReturnMasterView>("PurchaseInvoiceFilter", para, null, true, 0, commandType: CommandType.StoredProcedure).ToList();
				return ListofPlan;
			}
		}
		...
		public async Task<PurchaseReturnMaster> GetbyId(int id)
        {
			...
			{
				var para = new DynamicParameters();
				para.Add("@PurchaseReturnMasterId", id);
				para.Add("@TenantId", tenantId);
				var ListofPlan = sqlcon.Query<PurchaseReturnMaster>("SELECT *FROM PurchaseReturnMaster where PurchaseReturnMasterId=@PurchaseReturnMasterId AND TenantId=@TenantId", para, null, true, 0, commandType: CommandType.Text).FirstOrDefault();
				return ListofPlan;
			}
        }
        ...
        public async Task<string> GetSerialNo()
        {
            using (SqlConnection sqlcon = new SqlConnection(_conn.DbConn))
            {
                var para = new DynamicParameters();
                para.Add("@TenantId", tenantId);
                var val = sqlcon.Query<string>("SELECT ISNULL(MAX(SerialNo+1),1) as VoucherNo FROM PurchaseReturnMaster where TenantId=@TenantId", para, null, true, 0, commandType: CommandType.Text).FirstOrDefault();
                return val;
            }
        }

        public async Task<int> Approved(PurchaseReturnMaster master)
        {
			SqlConnection sqlcon = new SqlConnection(_conn.DbConn);
			sqlcon.Open();
			SqlTransaction sql = sqlcon.BeginTransaction();
		...
		public async Task<List<PurchaseReturnMaster>> PurchaseReturnBillView(int CustomerSupplierId)
		{
			using (SqlConnection sqlcon = new SqlConnection(_conn.DbConn))
			{
				var para = new DynamicParameters();
				...
				para.Add("@TenantId", tenantId);
				var ListofPlan = sqlcon.Query<PurchaseReturnMaster>("SELECT PurchaseMasterId,VoucherNo,Date,GrandTotal,BalanceDue,OrderNo FROM PurchaseMaster where CustomerSupplierId=@CustomerSupplierId AND TenantId=@TenantId and BalanceDue !=0 AND Status !='Draft'", para, null, true, 0, commandType: CommandType.Text).ToList();
				return ListofPlan;
			}
		}
			public async Task<List<ProductView>> PurchaseReturnDetailsView(int id)
        {
			using (SqlConnection sqlcon = new SqlConnection(_conn.DbConn))
			{
				var para = new DynamicParameters();
				para.Add("@PurchaseReturnMasterId", id);
				para.Add("@TenantId", tenantId);
				var ListofPlan = sqlcon.Query<ProductView>("PurchaseReturnDetailsView", para, null, true, 0, commandType: CommandType.StoredProcedure).ToList();
				return ListofPlan;
			}
		}

        public async Task<PurchaseReturnMasterView> PurchaseReturnInvoiceView(int id)
        {
        ...
        public async Task<bool> Update(PurchaseReturnMaster master)
        {
			SqlConnection sqlcon = new SqlConnection(_conn.DbConn);
			sqlcon.Open();
			SqlTransaction sql = sqlcon.BeginTransaction();
			...
			{
				var para = new DynamicParameters();
				para.Add("@PurchaseReturnMasterId", master.PurchaseReturnMasterId);
				para.Add("@SerialNo", master.SerialNo);
				para.Add("@VoucherNo", master.VoucherNo);
				para.Add("@WarehouseId", master.WarehouseId);
				para.Add("@OrderNo", master.OrderNo);
				...
				para.Add("@AddedDate", DateTime.Now);
				para.Add("@TenantId", tenantId);
				sqlcon.Execute("UPDATE PurchaseReturnMaster SET SerialNo=@SerialNo,VoucherNo=@VoucherNo,WarehouseId=@WarehouseId,OrderNo=@OrderNo,Date=@Date,CustomerSupplierId=@CustomerSupplierId,Narration=@Narration,PurchaseMasterId=@PurchaseMasterId,NetAmounts=@NetAmounts,TotalTax=@TotalTax,DisPer=@DisPer,DiscountType=@DiscountType,BillDiscount=@BillDiscount,GrandTotal=@GrandTotal,TotalAmount=@TotalAmount,PreviousDue=@PreviousDue,VoucherTypeId=@VoucherTypeId,UserId=@UserId,FinancialYearId=@FinancialYearId,BalanceDue=@BalanceDue,Status=@Status,PayAmount=@PayAmount,AddedDate=@AddedDate,TenantId=@TenantId,Reference=@Reference where PurchaseReturnMasterId=@PurchaseReturnMasterId", para, sql, 0, CommandType.Text);
				...
				foreach (var item in master.listOrder)
				{
					...
					{
						var paraOpening = new DynamicParameters();
						paraOpening.Add("@PurchaseReturnMasterId", master.PurchaseReturnMasterId);
						paraOpening.Add("@PurchaseDetailsId", 0);
						paraOpening.Add("@ProductId", item.ProductId);
						paraOpening.Add("@Qty", item.Qty);
						paraOpening.Add("@Rate", item.Rate);
					...
					else
					{
						...
						paraOpening.Add("@Description", item.Description);
						paraOpening.Add("@TenantId", tenantId);
						sqlcon.Execute("UPDATE PurchaseReturnDetails SET PurchaseReturnMasterId=@PurchaseReturnMasterId,PurchaseDetailsId=@PurchaseDetailsId,ProductId=@ProductId,Qty=@Qty,Rate=@Rate,UnitId=@UnitId,Discount=@Discount,DiscountAmount=@DiscountAmount,TaxId=@TaxId,BatchId=@BatchId,TaxAmount=@TaxAmount,GrossAmount=@GrossAmount,NetAmount=@NetAmount,LedgerId=@LedgerId,ProjectId=@ProjectId,TaxRate=@TaxRate,Amount=@Amount,Description=@Description,TenantId=@TenantId where PurchaseReturnDetailsId=@PurchaseReturnDetailsId", paraOpening, sql, 0, CommandType.Text);
				...
				paraLedgerDelete.Add("@DetailsId", master.PurchaseReturnMasterId);
				...
				foreach (var item in master.listOrder)
				{
					...
					paraSc.Add("@Credit", item.Amount);
					paraSc.Add("@VoucherNo", master.VoucherNo);
					paraSc.Add("@DetailsId", master.PurchaseReturnMasterId);
					paraSc.Add("@YearId", master.FinancialYearId);
					paraSc.Add("@InvoiceNo", master.OrderNo);
					paraSc.Add("@VoucherTypeId", master.VoucherTypeId);
					paraSc.Add("@LongReference", master.Narration);
				...
				paraPayable.Add("@Credit", 0);
				paraPayable.Add("@VoucherNo", master.VoucherNo);
				paraPayable.Add("@DetailsId", master.PurchaseReturnMasterId);
				paraPayable.Add("@YearId", master.FinancialYearId);
				paraPayable.Add("@InvoiceNo", master.OrderNo);
				paraPayable.Add("@VoucherTypeId", master.VoucherTypeId);
				paraPayable.Add("@LongReference", master.Narration);
				...
				paraTaxPayable.Add("@Credit", master.TotalTax);
				paraTaxPayable.Add("@VoucherNo", master.VoucherNo);
				paraTaxPayable.Add("@DetailsId", master.PurchaseReturnMasterId);
				paraTaxPayable.Add("@YearId", master.FinancialYearId);
				paraTaxPayable.Add("@InvoiceNo", master.OrderNo);
				paraTaxPayable.Add("@VoucherTypeId", master.VoucherTypeId);
				paraTaxPayable.Add("@LongReference", master.Narration);
				...
				paraDiscount.Add("@Credit", 0);
				paraDiscount.Add("@VoucherNo", master.VoucherNo);
				paraDiscount.Add("@DetailsId", master.PurchaseReturnMasterId);
				paraDiscount.Add("@YearId", master.FinancialYearId);
				paraDiscount.Add("@InvoiceNo", master.OrderNo);
				paraDiscount.Add("@VoucherTypeId", master.VoucherTypeId);
				paraDiscount.Add("@LongReference", master.Narration);
				...
				foreach (var item in master.listDelete)
				{
					...
					var value = sqlcon.Query<long>("DELETE FROM PurchaseReturnDetails where PurchaseReturnDetailsId=@PurchaseReturnDetailsId AND TenantId=@TenantId", paraDetailsDelete, sql, true, 0, commandType: CommandType.Text);
				}
				foreach (var item in master.listDelete)
				{
		...
		public async Task<bool> UpdateDraft(PurchaseReturnMaster master)
		{
			SqlConnection sqlcon = new SqlConnection(_conn.DbConn);
			sqlcon.Open();
			SqlTransaction sql = sqlcon.BeginTransaction();
		...
		public async Task<bool> ApprovedUpdate(PurchaseReturnMaster master)
		{
			SqlConnection sqlcon = new SqlConnection(_conn.DbConn);
			sqlcon.Open();
			SqlTransaction sql = sqlcon.BeginTransaction();
