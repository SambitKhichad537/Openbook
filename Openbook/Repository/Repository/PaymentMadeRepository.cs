...
{
	public class PaymentMadeRepository : IPaymentMade
	{
		private readonly ApplicationDbContext _context;
		private readonly DatabaseConnection _conn;
		private string tenantId;
		public PaymentMadeRepository(ApplicationDbContext context, DatabaseConnection conn, IServicioTenant servicioTenant)
		{
			_context = context;
			_conn = conn;
		...
		public async Task<bool> Delete(PaymentMaster master)
		{
			try
			{
				...
				var result = await (from a in _context.PaymentDetails
									where a.PaymentMasterId == master.PaymentMasterId && a.TenantId == tenantId
									select new
									{
				...
				if (result != null)
				{
					foreach (var item in result)
					{
						...
						using (SqlConnection sqlconn = new SqlConnection(_conn.DbConn))
						{
                            ...
                            purchase = sqlconn.Query<PurchaseMaster>("SELECT *FROM PurchaseMaster where PurchaseMasterId=@PurchaseMasterId and TenantId=@TenantId", paras, null, true, 0, commandType: CommandType.Text).FirstOrDefault();
						}
						if (purchase != null)
						{
							//UpdatePurchaseBalance
							using (SqlConnection sqlconnn = new SqlConnection(_conn.DbConn))
							{
								var paraPurchase = new DynamicParameters();
								paraPurchase.Add("@PurchaseMasterId", item.PurchaseMasterId);
                                ...
                                sqlconnn.Execute("UPDATE PurchaseMaster SET BalanceDue=@BalanceDue,Status=@Status,PaymentStatus=@PaymentStatus where PurchaseMasterId=@PurchaseMasterId AND TenantId=@TenantId", paraPurchase, null, 0, CommandType.Text);
							}
						}
					}
				}
				...
				SqlCommand cmd = new SqlCommand("PaymentMadeDelete", sqlcon);
		...
		public async Task<IList<PaymentMasterView>> GetAll(DateTime FromDate, DateTime ToDate, int SupplierId, string VoucherNo, string strType)
		{
			using (SqlConnection sqlcon = new SqlConnection(_conn.DbConn))
			{
				...
				var ListofPlan = sqlcon.Query<PaymentMasterView>("PaymentViewAll", para, null, true, 0, commandType: CommandType.StoredProcedure).ToList();
				return ListofPlan;
			}
		}
		public async Task<PaymentMasterView> GetbyIdView(int PaymentMasterId)
		{
			using (SqlConnection sqlcon = new SqlConnection(_conn.DbConn))
			{
				var para = new DynamicParameters();
				...
				para.Add("@TenantId", tenantId);
				var ListofPlan = sqlcon.Query<PaymentMasterView>("PaymentViewPrint", para, null, true, 0, commandType: CommandType.StoredProcedure).FirstOrDefault();
				return ListofPlan;
			}
		}
			public async Task<PaymentMaster> GetbyId(int id)
		{
			using (SqlConnection sqlcon = new SqlConnection(_conn.DbConn))
			{
				...
				para.Add("@TenantId", tenantId);
				var ListofPlan = sqlcon.Query<PaymentMaster>("SELECT *FROM PaymentMaster where PaymentMasterId=@PaymentMasterId AND TenantId=@TenantId", para, null, true, 0, commandType: CommandType.Text).FirstOrDefault();
				return ListofPlan;
			}
		}
		...
		public async Task<string> GetSerialNo()
		{
			using (SqlConnection sqlcon = new SqlConnection(_conn.DbConn))
			{
				var para = new DynamicParameters();
				para.Add("@TenantId", tenantId);
				var val = sqlcon.Query<string>("SELECT ISNULL(MAX(SerialNo+1),1) as VoucherNo FROM PaymentMaster where TenantId=@TenantId", para, null, true, 0, commandType: CommandType.Text).FirstOrDefault();
				return val;
			}
		}

		public async Task<IList<PaymentMaster>> PaymentMadePurchase(int id)
		{
			var result = await (from a in _context.PaymentMaster
								join b in _context.PaymentDetails on a.PaymentMasterId equals b.PaymentMasterId
								where b.PurchaseMasterId == id
		...
		public async Task<IList<PaymentDetailsView>> PaymentFor(int id)
		{
			var result = await (from a in _context.PaymentDetails
								join b in _context.PurchaseMaster on a.PurchaseMasterId equals b.PurchaseMasterId
		...
		public async Task<IList<PaymentDetailsView>> PaymentDetailsView(int id)
		{
			var result = await (from a in _context.PaymentDetails
								join b in _context.PurchaseMaster on a.PurchaseMasterId equals b.PurchaseMasterId
								where a.PaymentMasterId == id
			...
			try
			{
				...
				if (Id > 0)
				{
					...
					foreach (var item in model.listOrder)
					{
						...
						paraOpening.Add("@PaymentMasterId", Id);
						paraOpening.Add("@PurchaseMasterId", item.PurchaseMasterId);
						...
						sqlcon.Execute("DECLARE @ReturnValue int INSERT INTO PaymentDetails (PaymentMasterId,PurchaseMasterId,TotalAmount,ReceiveAmount,DueAmount,TenantId)VALUES(@PaymentMasterId,@PurchaseMasterId,@TotalAmount,@ReceiveAmount,@DueAmount,@TenantId) SELECT @ReturnValue = SCOPE_IDENTITY() set @DId =SCOPE_IDENTITY()", paraOpening, sql, 0, CommandType.Text);
                        ...
                        paraPurchase.Add("@TenantId", tenantId);
                        sqlcon.Execute("UPDATE PurchaseMaster SET BalanceDue=@BalanceDue,Status=@Status,PaymentStatus=@PaymentStatus where PurchaseMasterId=@PurchaseMasterId AND TenantId=@TenantId", paraPurchase, sql, 0, CommandType.Text);
					...
					paraCash.Add("@Credit", model.Amount);
					...
					paraPrepaidExpenses.Add("@LedgerId", 7);
					paraPrepaidExpenses.Add("@Debit", model.Amount);
					paraPrepaidExpenses.Add("@Credit", 0);
		...
		public async Task<bool> Update(PaymentMaster model)
		{
			...
			{
				var para = new DynamicParameters();
				para.Add("@PaymentMasterId", model.PaymentMasterId);
				para.Add("@SerialNo", model.SerialNo);
				para.Add("@VoucherNo", model.VoucherNo);
				para.Add("@LedgerId", model.LedgerId);
				para.Add("@CustomerSupplierId", model.CustomerSupplierId);
				...
				sqlcon.Execute("UPDATE PaymentMaster SET SerialNo=@SerialNo,VoucherNo=@VoucherNo,LedgerId=@LedgerId,CustomerSupplierId=@CustomerSupplierId,Date=@Date,PaidAmount=@PaidAmount,Amount=@Amount,Narration=@Narration,VoucherTypeId=@VoucherTypeId,UserId=@UserId,Type=@Type,PaymentType=@PaymentType,FinancialYearId=@FinancialYearId,TenantId=@TenantId,ModifyDate=@ModifyDate,Reference=@Reference where PaymentMasterId=@PaymentMasterId", para, sql, 0, CommandType.Text);

					foreach (var item in model.listOrder)
					{
						var paraOpening = new DynamicParameters();
					paraOpening.Add("@PaymentDetailsId", item.PaymentDetailsId);
					paraOpening.Add("@PaymentMasterId", item.PaymentMasterId);
						paraOpening.Add("@PurchaseMasterId", item.PurchaseMasterId);
						paraOpening.Add("@TotalAmount", item.TotalAmount);
						paraOpening.Add("@ReceiveAmount", item.ReceiveAmount);
						paraOpening.Add("@DueAmount", item.DueAmount);
						paraOpening.Add("@TenantId", tenantId);
						sqlcon.Execute("UPDATE PaymentDetails SET PaymentMasterId=@PaymentMasterId,PurchaseMasterId=@PurchaseMasterId,TotalAmount=@TotalAmount,ReceiveAmount=@ReceiveAmount,DueAmount=@DueAmount,TenantId=@TenantId where PaymentDetailsId=@PaymentDetailsId", paraOpening, sql, 0, CommandType.Text);
				...
				paraLedgerDelete.Add("@DetailsId", model.PaymentMasterId);
				...
				var paraCash = new DynamicParameters();
					...
					paraCash.Add("@Credit", model.Amount);
					paraCash.Add("@VoucherNo", model.VoucherNo);
					paraCash.Add("@DetailsId", model.PaymentMasterId);
					paraCash.Add("@YearId", model.FinancialYearId);
					paraCash.Add("@InvoiceNo", model.VoucherNo);
					paraCash.Add("@VoucherTypeId", model.VoucherTypeId);
					paraCash.Add("@LongReference", model.Narration);
					...
					paraPrepaidExpenses.Add("@Credit", 0);
					paraPrepaidExpenses.Add("@VoucherNo", model.VoucherNo);
					paraPrepaidExpenses.Add("@DetailsId", model.PaymentMasterId);
					paraPrepaidExpenses.Add("@YearId", model.FinancialYearId);
					paraPrepaidExpenses.Add("@InvoiceNo", model.VoucherNo);
					paraPrepaidExpenses.Add("@VoucherTypeId", model.VoucherTypeId);
					paraPrepaidExpenses.Add("@LongReference", model.Narration);
		...
		public async Task<decimal> Receiable()
		{
			using (SqlConnection sqlcon = new SqlConnection(_conn.DbConn))
			{
				var para = new DynamicParameters();
				para.Add("@TenantId", tenantId);
				var val = sqlcon.Query<decimal>("SELECT ISNULL(SUM(Debit),0) - ISNULL(SUM(Credit),0) as Receiable FROM LedgerPosting where LedgerId='5' AND TenantId=@TenantId", para, null, true, 0, commandType: CommandType.Text).FirstOrDefault();
				return val;
			}
		}
	}
